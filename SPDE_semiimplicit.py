...
utilising the semi-implicit numerical PDE scheme as detailed in Section 6 of the article: 
...
def SPDE_semiimplicit(model_params, global_params, X, M_VOL_TIME, M_TIME, M_SPACE, M_PATH, model, option):

    """
    Computes solution to the informal SPDE for Put or Call option utilising the semi-implicit
   ...
   # Feller test for Heston model
    ...
    for l in range(M_PATH_it + z_o):
        ...
        # Now all simluation of Z and V is done, we must consider the semi-implicit numerical PDE scheme.
        ...
        for i in range(M_TIME-1,-1,-1):
            ...
            # Now that unique model parameters are set, we do some manipulations
            ...
            # For matrix B: we create a dterm and then adjust it accordingly
        
            d_ii_t_p1 = .5*rhosigondx_ii*DeltaZ
            d_ii_t_m1 = -1.0*d_ii_t_p1 
                    
            d_ii_t_m1[M_SPACE-1,:] = 0
            d_ii_t_p1[1,:] = 0
            ...
            for k in range(M_PATH):   
                ...
                # Create matrices                
                B = spdiags((d_ii_t_m1[:,k], np.ones([M_SPACE+1]), d_ii_t_p1[:,k]), [-1,0,1], M_SPACE+1, M_SPACE+1)          
                A = -1.0*np.row_stack((c_i_cp[:,k], b_i_m1[:,k], a_i_cp[:,k]))
...
if __name__ == '__main__':
    ...
    SP = SPDE_semiimplicit(model_params, global_params, X, M_VOL_TIME, M_TIME, M_SPACE, M_PATH, model, option)

    print(f"\nPrice of European put option in model {model}:\n{SP[0]:.3f}")
    
    print(f"\nThe Monte Carlo Standard Error is {100*SP[1]:.3f}%")
    
    print(f"\nSPDE semi-implicit solving time in seconds {SP[2]:.3f}s\n")